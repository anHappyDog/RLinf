...
@pytest.mark.parametrize(
    ("grad_norm_before_clip", "max_grad_norm", "expected_after", "expected_coef"),
    [
        (0.5, 1.0, 0.5, 1.0),
        (4.0, 1.0, 1.0, 0.25),
        (0.0, 1.0, 0.0, 1.0),
...
def test_compute_gradient_clipping_metrics(
    grad_norm_before_clip,
    max_grad_norm,
    expected_after,
    expected_coef,
):
    metrics = compute_gradient_clipping_metrics(
        grad_norm_before_clip,
        max_grad_norm,
    )
    ...
    assert metrics == {
        ...
        "grad_norm_after_clip": expected_after,
        "grad_clip_coef": expected_coef,
    }


def test_compute_gradient_clipping_metrics_preserves_nonfinite_norm():
    metrics = compute_gradient_clipping_metrics(math.inf, 1.0)
