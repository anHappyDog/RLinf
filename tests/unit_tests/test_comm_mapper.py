...
from rlinf.data.embodied_io_struct import EnvOutput, PolicyOutput
...
def test_policy_output_split_preserves_actions():
    policy_output = PolicyOutput(
        actions=torch.arange(12, dtype=torch.float32).view(6, 2)
    )
    worker = object.__new__(MultiStepRolloutWorker)
    first, second = worker._split_policy_output(policy_output, [4, 2])

    assert torch.equal(first.actions, policy_output.actions[:4])
    assert torch.equal(second.actions, policy_output.actions[4:])
