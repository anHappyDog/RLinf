...
from toolkits.b1k_grounded.build_pilot_dataset import (
    GroundingAssessment,
    PilotBuildConfig,
    _iter_episode_results,
    _select_episode_candidates,
    _validate_source_episodes,
    assess_grounding,
    clip_annotation_interval,
    extract_action_chunk,
    pilot_arrow_schema,
    select_interval_frames,
    select_interval_frames_by_stride,
)
from toolkits.b1k_grounded.episode_split import (
    FORMAT_VERSION as SPLIT_FORMAT_VERSION,
)
from toolkits.b1k_grounded.episode_split import (
...
def test_episode_builder_rejects_nonpositive_worker_count():
    with pytest.raises(ValueError, match="num_workers must be positive"):
        list(_iter_episode_results([], num_workers=0))


def test_annotation_interval_clips_only_a_partial_tail():
    assert clip_annotation_interval(8001, 8541, 8525) == (8001, 8525, True)
    assert clip_annotation_interval(10, 20, 100) == (10, 20, False)
    with pytest.raises(IndexError, match="starts outside"):
        clip_annotation_interval(100, 120, 100)


def test_explicit_episode_selection_validates_files_and_tasks(tmp_path):
    annotation_dir = tmp_path / "annotations" / "task-0000"
    annotation_dir.mkdir(parents=True)
    (annotation_dir / "episode_00000010.json").write_text("{}")

    assert _validate_source_episodes(tmp_path, [(0, 10)], [0]) == [(0, 10)]
    with pytest.raises(ValueError, match="do not match"):
        _validate_source_episodes(tmp_path, [(0, 10)], [0, 12])
    with pytest.raises(FileNotFoundError, match="does not exist"):
        _validate_source_episodes(tmp_path, [(0, 20)], [0])


def test_episode_split_keeps_exposed_episodes_in_train(tmp_path):
    ...
    annotation_dir = dataset_root / "annotations" / "task-0000"
    annotation_dir.mkdir(parents=True)
    for episode_index in (10, 20, 30, 40):
        (annotation_dir / f"episode_{episode_index:08d}.json").write_text("{}")
    exclusion_manifest = tmp_path / "old_manifest.json"
    exclusion_manifest.write_text(
        json.dumps({"source_episodes": [{"task_index": 0, "episode_index": 10}]})
    )
    ...
    split = create_episode_split(
        ...
        test_per_task=1,
        seed=7,
        exclude_manifests=[exclusion_manifest],
        exclude_episodes=[(0, 20)],
    )

    assert split["format_version"] == SPLIT_FORMAT_VERSION
    assert {row["episode_index"] for row in split["splits"]["train"]} == {10, 20}
    assert not (
        {10, 20}
        & {
            row["episode_index"]
    ...
    episodes, metadata = load_episode_split(split_path, "train", task_indices=[0])
    assert episodes == [(0, 10), (0, 20)]
    assert metadata["split_manifest_sha256"]
...
def test_pilot_arrow_schema_fixes_state_and_action_shapes():
    ...
    assert schema.field("subgoal").nullable
    assert schema.field("subgoal_status").type == pa.string()
    assert schema.field("subgoal_primitive_indices").type == pa.list_(pa.int32())
