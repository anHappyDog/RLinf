...
def test_common_manifest_value_allows_missing_values_but_rejects_conflicts():
    manifests = [{"policy": "v1"}, {}, {"policy": "v1"}]

    assert _common_manifest_value(manifests, "policy") == "v1"
    assert _common_manifest_value(manifests, "missing") is None
    with pytest.raises(ValueError, match="different policy"):
        _common_manifest_value([{"policy": "v1"}, {"policy": "v2"}], "policy")
