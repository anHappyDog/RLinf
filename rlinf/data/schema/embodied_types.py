...
class ChunkStepResult:
    ...
    def __post_init__(self):
        ...
        if self.outcome_group_ids is not None:
            self.outcome_group_ids = self.outcome_group_ids.cpu().contiguous()
        if self.forward_inputs:
            self.forward_inputs = put_tensor_device(self.forward_inputs, "cpu")
        if self.versions is not None:
            self.versions = self.versions.cpu().contiguous()
...
class Trajectory:
    ...
    outcome_group_ids: torch.Tensor = None
    ...
    def extract_intervene_traj(self, mode="any"):
        ...
        for i in range(mask.shape[1]):
            ...
            outcome_group_ids = apply_mask(self.outcome_group_ids, i)
            ...
            filtered_trajectories.append(
                Trajectory(
                    ...
                    rewards=rewards,
                    successes=successes,
                    outcome_group_ids=outcome_group_ids,
