...
class EnvResult:
    """Environment result for the action in the preceding rollout request."""
...
@dataclass(kw_only=True)
class PolicyOutput:
    """Policy output consumed by an environment worker."""
...
def merge_policy_inputs(policy_inputs: list[PolicyInput]) -> PolicyInput:
    ...
    return PolicyInput(
        ...
        is_last=is_last,
        sources=[
            source for policy_input in policy_inputs for source in policy_input.sources
        ],
    )
...
class EnvOutput:
    ...
    def to_result(self, reward_model_output: torch.Tensor | None = None) -> EnvResult:
        """Return the data produced by the most recent environment action."""
        return EnvResult(
...
class TrajectorySegment:
    """One executed policy action and the environment result it produced."""
    ...
    def __post_init__(self) -> None:
        """Normalize every tensor that crosses the trajectory P2P boundary."""
        self.obs = put_tensor_device(self.obs, "cpu")
        self.next_obs = put_tensor_device(self.next_obs, "cpu")
        if self.next_forward_inputs is not None:
            self.next_forward_inputs = put_tensor_device(
                self.next_forward_inputs, "cpu"
            )


@dataclass(frozen=True, kw_only=True)
class TrajectoryEnd:
    """Marks the end of one rollout worker's contribution to a train step."""
...
class TrajectoryEpochEnd:
    """Marks one rollout epoch for pipeline micro-batch preparation."""
...
class EmbodiedRolloutResult:
    ...
    def append_initial_state(
        ...
        *,
        ...
        """Append the state boundary immediately before an rollout epoch."""
    ...
    def append_final_value(self, value: torch.Tensor | None) -> None:
        """Append the bootstrap value for the state after an rollout epoch."""
        if value is not None:
...
class EmbodiedLerobotRolloutResult(EmbodiedRolloutResult):
    ...
    TrajectoryWorker creates one collector per logical environment stage when
    ...
    **Integration**: When online LeRobot collection is enabled, TrajectoryWorker
    builds this class and the actor loads
    ...
    a rollout step, completed episodes flow through
    :meth:`drain_episodes` → ``TrajectoryWorker`` →
    ...
    **Runtime lifecycle inside TrajectoryWorker**:

    1. First rollout step: create one collector per logical environment stage.
    ...
    4. End of a rollout step: ``drain_episodes()`` and send to the actor.
    5. Collector lifetime spans rollout steps, so unfinished episodes remain local.
