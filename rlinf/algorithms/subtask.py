...
def summarize_outcome_group(
    outcome_shards: list[list[bool]],
) -> tuple[int, int]:
    """Count successful and failed trajectories returned by actor ranks."""
    outcomes = [bool(outcome) for shard in outcome_shards for outcome in shard]
    successes = sum(outcomes)
    return successes, len(outcomes) - successes


def outcome_group_is_trainable(
    outcome_shards: list[list[bool]],
    *,
    expected_size: int,
    min_successes: int,
    ...
    """Check whether an on-policy group contains enough positive and negative data."""
    successes, failures = summarize_outcome_group(outcome_shards)
    actual_size = successes + failures
    if actual_size != expected_size:
        raise ValueError(
            f"Expected {expected_size} rollout outcomes, received {actual_size}."
        )
    return (
        successes >= min_successes and failures >= min_failures,
        successes,
        failures,
    )


def reduce_trajectory_successes(successes: torch.Tensor) -> list[bool]:
    """Reduce per-step success flags to one outcome per trajectory."""
    if successes.ndim < 2:
        raise ValueError(
            "Rollout success outcomes must have time and batch dimensions."
        )
    batch_size = successes.shape[1]
    return (
        successes.to(torch.bool)
        .transpose(0, 1)
...
def reduce_first_episode_successes(
    terminations: torch.Tensor,
    dones: torch.Tensor,
) -> list[bool]:
    """Return the outcome of the first complete episode in each env stream.

    Auto-reset rollouts may pack multiple episodes into one fixed-length
    trajectory. Outcome-based sampling must compare the first episode from each
    same-state stream; reducing with ``any`` would instead favor streams that
    happened to fit more reset episodes.

    Args:
        terminations: Per-step successful-termination flags with shape
            ``[T, B, ...]``.
        dones: Per-step episode-end flags with shape ``[T, B, ...]``.

    Returns:
        One boolean outcome per batch element.
    """
    if terminations.ndim < 2 or dones.ndim < 2:
        raise ValueError("terminations and dones must have time and batch dimensions.")
    if terminations.shape[:2] != dones.shape[:2]:
        raise ValueError(
            "terminations and dones must share time and batch dimensions, got "
            f"{tuple(terminations.shape[:2])} and {tuple(dones.shape[:2])}."
        )

    termination_flags = (
        terminations.to(torch.bool).reshape(*terminations.shape[:2], -1).any(-1)
    )
    done_flags = dones.to(torch.bool).reshape(*dones.shape[:2], -1).any(-1)
    if not done_flags.any(dim=0).all():
        missing = (~done_flags.any(dim=0)).nonzero(as_tuple=False).reshape(-1).tolist()
        raise ValueError(
            "Every auto-reset rollout stream must contain a complete first episode; "
            f"missing batch indices {missing}."
        )

    first_done = done_flags.to(torch.int64).argmax(dim=0)
    batch_indices = torch.arange(done_flags.shape[1], device=done_flags.device)
    return termination_flags[first_done, batch_indices].tolist()


def reduce_trajectory_group_ids(group_ids: torch.Tensor) -> list[int]:
    """Reduce constant per-step group IDs to one ID per trajectory."""
    if group_ids.ndim < 2:
        raise ValueError("Rollout group IDs must have time and batch dimensions.")
    batch_size = group_ids.shape[1]
    flattened = group_ids.transpose(0, 1).reshape(batch_size, -1)
    first_ids = flattened[:, :1]
    if not torch.equal(flattened, first_ids.expand_as(flattened)):
        raise ValueError("Each trajectory must have one constant rollout group ID.")
    return [int(group_id) for group_id in first_ids.squeeze(1).tolist()]


def outcome_actor_channel_key(actor_rank: int) -> str:
    """Return the trajectory-channel key assigned to an actor rank."""
    return f"outcome_actor_{actor_rank}"


def parallel_outcome_sampling_enabled(sampling_cfg) -> bool:
    """Require dynamic sampling before enabling parallel keyed routing."""
    return bool(sampling_cfg.get("enabled", False)) and bool(
        sampling_cfg.get("parallel_groups", False)
    )
