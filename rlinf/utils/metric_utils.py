...
def compute_gradient_clipping_metrics(
    ...
    """Report gradient norm and clipping without ambiguous metric names."""
    if not math.isfinite(grad_norm_before_clip):
        return {
            "grad_norm_before_clip": grad_norm_before_clip,
            "grad_norm_after_clip": math.nan,
            "grad_clip_coef": math.nan,
        }

    grad_clip_coef = 1.0
    if grad_norm_before_clip > max_grad_norm:
        grad_clip_coef = max_grad_norm / grad_norm_before_clip
    return {
        "grad_norm_before_clip": grad_norm_before_clip,
        "grad_norm_after_clip": grad_norm_before_clip * grad_clip_coef,
