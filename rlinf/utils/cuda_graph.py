...
class CUDAGraphManager:
    ...
    def capture(
        ...
        _assert_on_cuda_tree(spec.outputs, name=f"{spec.name}.outputs")
        # Warmup iterations to ensure CUDA kernels are compiled
        for _ in range(spec.warmup_iters):
            spec.func(spec.inputs, spec.outputs)
        ...
        with torch.cuda.graph(graph, pool=pool):
            _ = spec.func(spec.inputs, spec.outputs)
