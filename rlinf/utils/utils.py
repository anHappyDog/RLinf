...
def preprocess_embodied_batch(
    ...
    *,
    ...
    if auto_reset and reward_type == "subtask_chunk_level":
        executed_action_mask = batch.get("executed_action_mask")
        if executed_action_mask is None:
            raise ValueError(
                "Auto-reset subtask rollouts require executed_action_mask."
            )
        loss_mask = executed_action_mask.to(torch.bool).any(dim=-1, keepdim=True)
        batch["loss_mask"] = loss_mask
        batch["loss_mask_sum"] = loss_mask.sum(dim=0, keepdim=True).expand_as(loss_mask)
