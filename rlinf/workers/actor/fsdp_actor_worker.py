...
class FSDPActor(FSDPModelManager, Worker):
    def __init__(
        ...
        self.is_async = self._component_placement.is_async
        ...
        self.enable_offload = (
            self.cfg.actor.get("enable_offload", False)
            and not self.is_pipeline
        ...
        self.loss_type = self.cfg.algorithm.loss_type
        self.entropy_op_type = self.cfg.algorithm.get("entropy_op_type", "liger_kernel")
        self.behave_weight_threshold = self.cfg.algorithm.get(
    ...
    def sync_model_to_rollout(self) -> None:
        ...
        if self._weight_dst_rank_in_rollout is not None:
            for k, v in self.rollout_state_dict.items():
                ...
                state_dict[name] = (
                    reduce_tensor(v)
                    if not self.is_pipeline and not self.is_async
                    else v
                )
            if not self.is_pipeline and not self.is_async:
                self.send(
        ...
        for idx, m_batch in enumerate(micro_batches_iter):
            ...
            with self.amp_context:
                ...
                loss, mbs_metrics_data = policy_loss(
                    ...
                    behave_weight_threshold=self.behave_weight_threshold,
                )
