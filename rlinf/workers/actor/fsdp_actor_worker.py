...
def _merge_pipeline_train_batches(batches: list):
    ...
    if isinstance(first_batch, dict):
        return {
            key: _merge_pipeline_train_batches([batch[key] for batch in valid_batches])
            for key in first_batch
        }
    raise NotImplementedError(
        f"Unsupported pipeline train batch type for merging: {type(first_batch)}"
    )
