...
class AsyncPPOEnvWorker(EnvWorker):
    ...
    def init_env_outputs(self) -> list[EnvOutput]:
        ...
        if not self.cfg.env.train.auto_reset:
            ...
            last_dones = getattr(self, "last_dones_list", [zero_dones] * self.stage_num)
            last_terminations = getattr(
                self, "last_terminations_list", [zero_dones] * self.stage_num
            )
            last_truncations = getattr(
                self, "last_truncations_list", [zero_dones] * self.stage_num
            )
            for stage_id in range(self.stage_num):
                env_output = EnvOutput(
                    obs=self.last_obs_list[stage_id],
                    rewards=None,
                    dones=last_dones[stage_id],
                    terminations=last_terminations[stage_id],
                    truncations=last_truncations[stage_id],
