...
class BehaviorEnv(gym.Env):
    ...
    def step(
        ...
        """
        Do a single step with action tokens generated by rollout worker, using Behavior Simulator.

        Args:
            actions (torch.Tensor): [num_envs, action_dim]: actions to take in the environment
    ...
    def chunk_step(self, chunk_actions: torch.Tensor) -> tuple:
        """
        Do a chunked step in the environment with batched chunk_actions generated
        by rollout worker.

        args:
            chunk_actions(torch.Tensor): [num_envs, chunk_steps, action_dim]: chunked actions

        Returns:
            extracted_obs(dict[str,torch.Tensor]): a dict
            chunk_rewards(torch.Tensor): [num_envs, chunk_steps]: batched rewards for every chunk step
            chunk_terminations(torch.Tensor): [num_envs, chunk_steps]: batched terminations for every chunk step
            chunk_truncations(torch.Tensor): [num_envs, chunk_steps]: batched truncations for every chunk step
            infos(dict): info dict
        """
