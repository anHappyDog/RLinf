...
- ``omni_config.task.instance_resample_mode``：
  ...
  ``omni_config.task.activity_instance_dir``，按 OmniGibson 现有的
  ``<scene>_task_<activity>_<definition>_<instance>_template-tro_state.json``
  命名规则解析可用的 instance id，并在每次 ``env.reset()`` 前随机选取一个
  cached offline instance，且不 reload scene。
...
- ``omni_config.task.activity_instance_dir``：
  可选目录，里面存放符合 OmniGibson 标准命名规则的
  ``*-tro_state.json`` 文件。它既可用于 ``instance_resample_mode:
  offline``，也可用于 ``disabled`` 模式下按固定 ``activity_instance_id``
  加载离线 instance。
