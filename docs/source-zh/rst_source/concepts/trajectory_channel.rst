...
.. list-table::
   ...
   * - ``TrajectoryStorageWorker``
     - 接收训练记录，在后台聚合成 ``TrajectoryBatch`` 或 ``LeRobotEpisodeBatch``，再交给 actor。
...
先在 embodied 配置中启用 channel，并为 ``trajectory_channel`` 和 ``trajectory_storage`` 指定 placement。下面是单节点的最小配置；多节点时把节点编号替换为与你的 actor、环境和 rollout 部署相匹配的 placement。
...
该命令会加载 ``examples/embodiment/config/<config_name>.yaml``。当 ``trajectory_channel.enabled`` 为 ``true`` 时，``train_embodied_agent.py`` 和 ``train_async.py`` 会在创建工作组后、训练开始前创建 ``TrajectoryChannel``；runner 随后把 env、rollout、actor 和可选 reward 所需的路由视图传入各自的 worker。

.. warning::

   ``runner.use_training_pipeline: true`` 目前不能与 ``TrajectoryChannel`` 一起使用。保留该选项为 ``false``。此外，``env.train.total_num_envs`` 必须能被 actor world size 整除，否则 storage 无法为每个 actor 构造固定的轨迹分片。
...
.. list-table::
   ...
   * - ``trajectory_channel.enabled``
     - ``false``
     - 创建并使用 ``TrajectoryChannel``。关闭时，训练沿用原有的 ``Channel`` 数据路径。
   ...
   * - ``cluster.component_placement`` 中的 ``trajectory_channel``
     - 必填
     - 放置在线请求/响应队列 worker。建议优先与高频的环境或 rollout 流量同节点部署。
   * - ``cluster.component_placement`` 中的 ``trajectory_storage``
     - 必填
     - 放置轨迹聚合 worker。建议优先与 actor 同节点部署，减少完整训练批次的跨节点传输。
...
对于较大的高频 tensor，``PolicyInput``、``ValueRequest``、``RewardRequest``、``TrajectoryBatch`` 和 LeRobot 数据会在传输前尝试 LZ4 压缩。默认仅压缩不小于 64 KiB 的连续 CPU tensor，按 1 MiB 分块，并在压缩收益不足时保留原始数据。这个实现细节不需要额外配置，但意味着自定义的参与方在直接发布压缩消息前应保证 tensor 位于 CPU 且连续。
...
.. list-table::
   ...
   * - 启用后立即报不支持 training pipeline
     - 将 ``runner.use_training_pipeline`` 设为 ``false``；该组合尚未实现。
   * - 消费者等待或训练批次迟迟不出现
