...
def audit_sidecar(
    ...
    *,
    ...
    subgoal_statuses: collections.Counter[str] = collections.Counter()
    errors: list[str] = []
    rows = 0
    fully_grounded = 0
    part_samples = 0
    ...
    for shard in manifest["shards"]:
        ...
        present_subgoal_columns = subgoal_columns.intersection(
            parquet_file.schema_arrow.names
        )
        if present_subgoal_columns and present_subgoal_columns != subgoal_columns:
            record_error(f"Shard has incomplete subgoal metadata: {parquet_path}.")
        has_subgoal_metadata = present_subgoal_columns == subgoal_columns
        for batch in parquet_file.iter_batches(batch_size=2048):
            for row in batch.to_pylist():
                ...
                valid_length = min(expected_action_horizon, interval_end - frame_index)
                expected_padding = np.arange(expected_action_horizon) >= valid_length
                ...
                try:
                    ...
                    record_error(
                        f"Sample {sample_id} has invalid control_json: {error}"
                    )
                else:
                    ...
                    if has_subgoal_metadata:
                        if control.subgoal != row["subgoal"]:
                            record_error(
                                f"Sample {sample_id} has a mismatched subgoal."
                            )
                        try:
                            resolution = ResolvedSubgoal(
                                text=row["subgoal"],
                                status=SubgoalStatus(row["subgoal_status"]),
                                primitive_indices=tuple(
                                    row["subgoal_primitive_indices"]
                                ),
                            )
                        except (TypeError, ValueError) as error:
                            record_error(
                                f"Sample {sample_id} has invalid subgoal "
                                f"metadata: {error}"
                            )
                        else:
                            subgoal_statuses[resolution.status.value] += 1

                is_fully_grounded = (
                    row["object_grounding_complete"] and row["part_grounding_complete"]
                )
    ...
    expected_subgoal_statuses = manifest.get("subgoal_status_counts")
    if expected_subgoal_statuses is not None and (
        dict(sorted(subgoal_statuses.items())) != expected_subgoal_statuses
    ):
        record_error(
            "Observed subgoal status counts differ from the manifest: "
            f"{dict(sorted(subgoal_statuses.items()))} != "
            f"{expected_subgoal_statuses}."
        )
