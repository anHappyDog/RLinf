...
def _common_manifest_value(manifests: list[dict[str, Any]], key: str) -> Any | None:
    """Return a shared non-null manifest value and reject conflicts."""
    values = [
        manifest.get(key) for manifest in manifests if manifest.get(key) is not None
    ]
    if not values:
        return None
    first = values[0]
    if any(value != first for value in values[1:]):
        raise ValueError(f"Input datasets use different {key} values.")
    return first
...
def compose_pilot_datasets(
    ...
    *,
    ...
    format_versions = {manifest["format_version"] for manifest in source_manifests}
    if len(format_versions) != 1:
        raise ValueError("Input datasets use different sidecar format versions.")
    source_config = _common_manifest_value(source_manifests, "config")
    episode_selection = _common_manifest_value(source_manifests, "episode_selection")
    subgoal_resolution_policy = _common_manifest_value(
        source_manifests, "subgoal_resolution_policy"
    )
    annotation_interval_policy = _common_manifest_value(
        source_manifests, "annotation_interval_policy"
    )
    ...
    subgoal_statuses = collections.Counter(
        row["subgoal_status"] for row in rows if "subgoal_status" in row
    )
    ...
    source_episodes = sorted(
        {
            ...
            for record in manifest.get("source_episodes", [])
        }
    )
    annotation_interval_repairs = [
        repair
        for manifest in source_manifests
        for repair in manifest.get("annotation_interval_repairs", [])
    ]
    composition_config = dict(source_config or {})
    composition_config.update(
        {
    ...
    manifest = {
        "format_version": format_versions.pop(),
        ...
        "source_episodes": [
            ...
            for task_index, episode_index in source_episodes
        ],
        ...
        "subgoal_status_counts": dict(sorted(subgoal_statuses.items())),
        "task_sample_counts": {
