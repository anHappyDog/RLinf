...
MODE=${1:?Usage: $0 <calibrate|evaluate|evaluate-existing> [first_gpu] [last_gpu]}
    ...
    for ((gpu = FIRST_GPU; gpu <= LAST_GPU; gpu++)); do
        (
            for ((stage = gpu; stage < NUM_STAGES; stage += NUM_GPUS)); do
                ...
                "$TARGET_SCRIPT" "$MODE" "$gpu" "$stage" "$port" >"$log" 2>&1
            done
        ) &
        lane_pids+=("$!")
    done
...
wait_for_servers() {
    for ((lane = 0; lane < NUM_EVAL_LANES; lane++)); do
        ...
        for _ in {1..180}; do
            ...
            if ((${#SERVER_PIDS[@]} > lane)) && ! kill -0 "${SERVER_PIDS[$lane]}" 2>/dev/null; then
