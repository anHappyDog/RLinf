...
class GroundedSubtaskEvaluator(b1k_eval.Evaluator):
            ...
            with og.sim.render_on_step(False):
                while not replay_policy.is_done:
                    ...
                    if is_successful_predicate_termination(terminated, truncated, info):
                        ignored_success_terminations += 1
                        continue
                    raise RuntimeError(
    ...
    def calibrate_demo_segment(
        ...
        *,
        ...
        if start_frame > 0:
            warmup_policy = b1k_eval.ParquetDemoReplayPolicy(
                parquet_path=parquet_path,
                start_frame=0,
            ...
            with og.sim.render_on_step(False):
                while not warmup_policy.is_done:
                    terminated, truncated, _, _ = self._fast_step_with_policy(
                        warmup_policy
                    )
    ...
    with GroundedSubtaskEvaluator(config) as evaluator:
        for episode_index in episode_indices:
            ...
            for target in targets:
                ...
                reward_spec = resolve_demo_reward_spec(
                    reward_specs[skill],
                    start_frame=start_frame,
                    end_frame=end_frame,
                )
                ...
                evaluator.reset()
                state_init_mode = str(
                    config.get("subpool_state_init_mode", "demo_replay")
                )
                if state_init_mode == "demo_replay":
                ...
                elif state_init_mode == "official_subtask":
                    ...
                    evaluator._activate_target_subtask(  # noqa: SLF001
                        demo_data_dir=config.demo_data_dir,
                        task_index=task_index,
                        episode_index=episode_index,
                        start_frame=start_frame,
                    ...
                    raise ValueError(
                        "subpool_state_init_mode must be 'demo_replay' or "
                        f"'official_subtask', got {state_init_mode!r}."
                    )
                ...
                require_gt_success = bool(
                    config.get("subpool_require_gt_suffix_success", True)
                )
                if not success and require_gt_success:
                    raise RuntimeError(
                ...
                if not success:
                    b1k_eval.logger.warning(
                        "Exporting official subtask state despite non-replayable "
                        "open-loop GT suffix: episode=%d, segment=%d, skill=%s, "
                        "stage_info=%s",
                        episode_index,
                        segment_index,
                        skill,
                ...
                b1k_eval.logger.info(
                    "Exported canonical snapshot %s (gt_suffix_success=%s)",
                    snapshot_id,
                    success,
                )
