...
from toolkits.b1k_grounded.episode_split import load_episode_split
...
ANNOTATION_INTERVAL_POLICY = "clip_partial_tail_to_episode_length_v1"
...
class _PendingSample:
    ...
    subgoal_status: SubgoalStatus
    subgoal_primitive_indices: tuple[int, ...]
    skill_id: int
    skill_type: str
...
@dataclasses.dataclass(frozen=True)
class _EpisodeBuildResult:
    task_index: int
    episode_index: int
    candidate_count: int
    selected_candidates: list[tuple[dict[str, Any], GroundingAssessment]]
    parse_status_counts: dict[str, int]
    annotation_interval_repairs: list[dict[str, int]]
...
def clip_annotation_interval(
    start: int, end: int, episode_length: int
) -> tuple[int, int, bool]:
    """Clip a partially valid annotation tail to the recorded episode length."""
    if episode_length <= 0:
    ...
    if start >= episode_length:
        raise IndexError(
            f"Annotation interval [{start}, {end}) starts outside an episode "
            f"with {episode_length} frames."
        )
    clipped_end = min(end, episode_length)
    return start, clipped_end, clipped_end != end
    ...
    *, state_dim: int, action_dim: int, action_horizon: int
    ...
    return pa.schema(
        [
            ...
            pa.field("subgoal", pa.string(), nullable=True),
            pa.field("subgoal_status", pa.string(), nullable=False),
            pa.field(
...
def _validate_source_episodes(
    dataset_root: Path,
    episodes: list[tuple[int, int]],
    selected_task_indices: list[int],
) -> list[tuple[int, int]]:
    """Validate an explicit episode selection and return stable ordering."""
    episodes = sorted(episodes)
    if not episodes:
        raise ValueError("Explicit episode selection must not be empty.")
    if len(set(episodes)) != len(episodes):
        raise ValueError("Explicit episode selection contains duplicates.")
    selected_tasks = set(selected_task_indices)
    episode_tasks = {task_index for task_index, _ in episodes}
    if episode_tasks != selected_tasks:
        raise ValueError(
            "Explicit episode tasks do not match task_indices: "
            f"episodes={sorted(episode_tasks)}, requested={selected_task_indices}."
        )
    for task_index, episode_index in episodes:
        ...
        if not annotation_path.is_file():
            raise FileNotFoundError(
                f"Explicit episode annotation does not exist: {annotation_path}."
            )
    return episodes
...
def _pending_samples(
    ...
    *,
...
) -> tuple[list[_PendingSample], list[dict[str, int]]]:
    pending = []
    annotation_interval_repairs = []
    subgoals = resolve_episode_subgoals(annotation)
    for record in annotation["skill_annotation"]:
        skill_index = record.get("skill_idx") if isinstance(record, dict) else None
        resolved_subgoal = subgoals.get(skill_index)
        result = parse_skill_annotation(
            record,
            goal=goal,
            subgoal=(None if resolved_subgoal is None else resolved_subgoal.text),
        ...
        if result.status is not ParseStatus.VALID:
            continue
        if resolved_subgoal is None:
            raise RuntimeError(
                f"Valid skill {skill_index} has no subgoal resolution record."
            )
        segment = result.segment
        for interval_index, (source_start, source_end) in enumerate(
            segment.frame_intervals
        ):
            start, end, repaired = clip_annotation_interval(
                source_start, source_end, episode_length
            )
            if repaired:
                annotation_interval_repairs.append(
                    {
            ...
            for frame_index, fraction in selected_frames:
                pending.append(
                    _PendingSample(
                        ...
                        subgoal_status=resolved_subgoal.status,
                        subgoal_primitive_indices=(resolved_subgoal.primitive_indices),
    ...
    return pending, annotation_interval_repairs
...
def _build_row(
    ...
    *,
    ...
    row = {
        ...
        "goal": grounded.goal,
        "subgoal": grounded.subgoal,
        "subgoal_status": pending.subgoal_status.value,
        "subgoal_primitive_indices": list(pending.subgoal_primitive_indices),
...
def _build_episode(request: _EpisodeBuildRequest) -> _EpisodeBuildResult:
    """Build all selected rows for one episode."""
    ...
    pending, annotation_interval_repairs = _pending_samples(
        annotation,
        goal=request.task["task"],
        episode_index=episode_index,
    ...
    return _EpisodeBuildResult(
        ...
        annotation_interval_repairs=annotation_interval_repairs,
    )


def _iter_episode_results(requests: list[_EpisodeBuildRequest], num_workers: int):
    """Yield episode results in request order, optionally using subprocesses."""
    if num_workers <= 0:
        raise ValueError("num_workers must be positive.")
    if num_workers == 1:
        for request in requests:
            yield _build_episode(request)
        return

    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        yield from executor.map(_build_episode, requests)
...
def build_pilot_dataset(
    ...
    *,
    ...
    num_workers: int = 1,
    source_episodes: list[tuple[int, int]] | None = None,
    ...
    if source_episodes is None:
        ...
        episode_selection = {"mode": "first_n_per_task"}
    else:
        episodes = _validate_source_episodes(
            dataset_root, source_episodes, selected_task_indices
        )
        if episode_selection is None:
            episode_selection = {"mode": "explicit_api"}
    requests = [
        _EpisodeBuildRequest(
    ...
    subgoal_status_counts: collections.Counter[str] = collections.Counter()
    ...
    episode_results = _iter_episode_results(requests, num_workers)
    for episode_number, result in enumerate(episode_results, start=1):
        task_index = result.task_index
        parse_status_counts.update(result.parse_status_counts)
        annotation_interval_repairs.extend(
            {
                "task_index": result.task_index,
                "episode_index": result.episode_index,
                **repair,
            }
            for repair in result.annotation_interval_repairs
        )
        ...
        for row, assessment in selected_candidates:
            ...
            subgoal_status_counts[row["subgoal_status"]] += 1
        ...
        print(
            ...
            f"episode={result.episode_index:08d} "
    ...
    manifest = {
        ...
        "annotation_interval_policy": ANNOTATION_INTERVAL_POLICY,
        ...
        "counts": {
            ...
            "annotation_intervals_repaired": len(annotation_interval_repairs),
...
def main() -> None:
    ...
    parser.add_argument(
        "--episode-split",
        type=Path,
        help="Episode split manifest; requires --split-name.",
    )
    parser.add_argument(
        "--split-name",
        choices=("train", "validation", "test"),
        help="Section of --episode-split to build.",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
        default=1,
        help="Number of episode-building subprocesses.",
    )
    args = parser.parse_args()
    if (args.episode_split is None) != (args.split_name is None):
        parser.error("--episode-split and --split-name must be provided together.")
    source_episodes = None
    episode_selection = None
    if args.episode_split is not None:
        source_episodes, episode_selection = load_episode_split(
            args.episode_split,
            args.split_name,
