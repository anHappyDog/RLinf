...
export XDG_CACHE_HOME="/mnt/public/daibo/cache/$(hostname)/rlt_stage2"
...
export B1K_SUBPOOL_RESULT_DIR=${B1K_SUBPOOL_RESULT_DIR:-/mnt/public/daibo/results/b1k_grounded_control_v01/rlt_stage2_v1}
...
export B1K_RLT_RESULT_DIR=${B1K_RLT_RESULT_DIR:-/mnt/public/daibo/results/b1k_grounded_control_v01/rlt/stage2_v1}
: "${B1K_RLT_STAGE1_CHECKPOINT:?Point to the trained Stage1 actor checkpoint}"
cd "$repo"
if [[ ${1:-} == --config-only ]]; then
    shift
    exec "$venv/bin/python" examples/embodiment/train_embodied_agent.py --config-name behavior_rlt_stage2 --cfg job --resolve "$@"
fi
# This script never creates/stops Ray or collectors. Placement is user configurable.
exec "$venv/bin/python" examples/embodiment/train_embodied_agent.py --config-name behavior_rlt_stage2 "$@"
